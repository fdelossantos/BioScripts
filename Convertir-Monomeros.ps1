...
param (
    [Parameter()]
    [string]
    $InputFile = "C:\Users\feder\Facultad de Ingenieria - Universidad ORT Uruguay\2024 Obl Bioinfo2 CM-FD - General\8JD3\t1r3 ajustar.txt",
    [Parameter()]
    [string]
    $OuputFile = "C:\Users\feder\Facultad de Ingenieria - Universidad ORT Uruguay\2024 Obl Bioinfo2 CM-FD - General\8JD3\t1r3 ajustado.txt",
    [Parameter()]
    [int]
    $start = 6417,
    [Parameter()]
...
$archivo | ForEach-Object {
    ...
    $col3 = $_.Substring(11, 11)

    $col4 = [int]$_.Substring(22, 4).Trim()

    $col5 = $_.Substring(26, $_.Length - 26)

    # Sumar $start a la segunda columna
    $newCol2 = $col2 + $start
    $newCol4 = $col4 + $AminoStart

    # Formatear la nueva segunda columna para mantener el ancho fijo de 5 caracteres
    $newCol2 = "{0,5}" -f $newCol2
    $newCol4 = "{0,4}" -f $newCol4

    # Construir la nueva línea
    $newLine = $col1 + $newCol2 + $col3 + $newCol4 + $col5 + "  "
